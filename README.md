...
**[Try the dashboard](https://dashboard-attrition-prediction.streamlit.app/)** — the plain-English, no-code version.

**[Try the API docs](https://bigger-attrition-prediction-api-com.onrender.com/docs)** — the raw endpoint, for anyone technical.

Note: both run on free hosting tiers, so the very first request after a period of inactivity can take 30-50 seconds to wake up. That's expected, not a bug.
...
- FastAPI — serves predictions over a real endpoint, deployed on Render
- Streamlit — the dashboard that talks to the API, deployed on Streamlit Cloud
